...
//         `https://tms-backend-x26c.onrender.com/api/v1/temple/sevas/${id}`,
//         {
//           headers: {
...
export default function EditSeva() {
  ...
  const handleSubmit = async (formData: any) => {
    ...
    // 🔥 Convert select values → boolean (important)
    const payload = {
      ...formData,
      is_default: formData.is_default === "yes",
      is_recurring: formData.is_recurring === "yes",
    }

    try {
      await api.put(`/v1/temple/sevas/${id}`, payload)
    ...
    <>
      ...
      <FormBuilder
        ...
        defaultValues={{
          ...
          is_recurring: data?.is_recurring ? "yes" : "no",
        }}
        onSubmit={handleSubmit}
        errors={errors}
        fields={[
